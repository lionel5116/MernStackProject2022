  ...
  async (req, res) => {
    ...
    if (skills) {
      if (Array.isArray(skills)) {
        ...
        arrFinalSkills = arrSkills.substring(0, arrSkills.length - 1);
        profileFields.skills = arrFinalSkills
          .split(',')
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      const profile = await Profile.findOne({ user: req.user.id });
      profile.education.unshift(newEdu); //unshift is the same as push to front of array
      await profile.save();
      res.json(profile);
    } catch (err) {
      console.log(err.message);
...
//@ desc Delete education from profile with profile internal ID
//@access private
  ...
  try {
    const options = {
      uri: `https://api.github.com/users/${req.params.username}/repos?per_page=5
            ...
            &client_secre=${config.get('githubSecret')}`,
      method: 'GET',
